...
public class SAEvaluate {
    ...
    public void evaluate(String gold, String answer) {
        ...
        for (int i = 0; i < goldSA.size(); i++) {
            ...
            for (int j = 0; j < asp.size(); j++) {
                ...
                if (id != -1) {
                    goldAspCount.set(id, goldAspCount.get(id) + 1);
                } else {
                    System.out.println("!!! ERROR " + goldSA.get(i).id);
                }
        ...
        for (int i = 0; i < ansSA.size(); i++) {
            ...
            for (int j = 0; j < asp.size(); j++) {
                int id = allAspects.indexOf(asp.get(j));
                if (id != -1) {
                    ansAllAspCount.set(id, ansAllAspCount.get(id) + 1);
                } else {
                    System.out.println("!!! Warning " + asp.get(j));
                }
        ...
        for (int i = 0; i < goldSA.size(); i++) {
            ...
            if (!g.id.equals(a.id)) {
                System.out.println("Lỗi gióng hàng:" + g.id + " <-> " + a.id);
            } else {
                if (g.text.compareTo(a.text) != 0) {
                    // System.out.println("Lỗi văn bản:"+a.id+"\n["+g.text+"]\n
                    // <-> \n["+a.text+"]" );
                }
                ...
                for (int t = 0; t < gasp.size(); t++) {
                    ...
                    if (id != -1) {
                        ...
                        if (!checkDuplicateAsp(aspect, aasp)) {
                            if (gval.get(t).equals(aval.get(id))) {
                                ansValueCount.set(gid, ansValueCount.get(gid) + 1);
                            }
                        } else {
        ...
        for (int i = 0; i < allAspects.size(); i++) {
            System.out.printf("\t%s", "asp#" + (i + 1));
        }
        System.out.println();

        System.out.printf("%30s", "Gold count");
        for (int i = 0; i < allAspects.size(); i++) {
            System.out.printf("\t%d", goldAspCount.get(i));
        }
        System.out.println();

        System.out.printf("%30s", "ANSWER count");
        for (int i = 0; i < allAspects.size(); i++) {
            System.out.printf("\t%d", ansAllAspCount.get(i));
        }
        System.out.println();
        System.out.println();
        System.out.printf("%30s", "Correct ANSWER: aspect");
        for (int i = 0; i < allAspects.size(); i++) {
            System.out.printf("\t%d", ansAspCount.get(i));
        }
        System.out.println();
        ...
        for (int i = 0; i < allAspects.size(); i++) {
            ...
            if (ansAllAspCount.get(i) > 0)
                p = 1.0 * ansAspCount.get(i) / ansAllAspCount.get(i);
        ...
        for (int i = 0; i < allAspects.size(); i++) {
            double r = 0;
            if (goldAspCount.get(i) > 0)
                r = 1.0 * ansAspCount.get(i) / goldAspCount.get(i);
    ...
    public void evaluateFolder(String gold, String answer) {
        ...
        for (int i = 0; i < ansfiles.length; i++) {
            ...
            if (af.contains("hotel"))
                evaluate(gold + "/" + goldfiles[0], answer + "/" + ansfiles[i]);
            else if (af.contains("restaurant"))
                evaluate(gold + "/" + goldfiles[1], answer + "/" + ansfiles[i]);
        }
